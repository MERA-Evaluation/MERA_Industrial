...
# Turn off certificate checks for all HTTPS connections:
...
@register_task("agro_mc")
class AgroMC(Task):
    MULTIMODAL = True
    VERSION = 0
    DATA_PATH = os.path.join(os.path.dirname(__file__), "../data/SSU_AgroQ_Bench.jsonl")
    DATASET_NAME = "agro_mc"
    ...
    def doc_to_text(self, doc):
        # return the raw list of messages for openai-chat-completions
        msgs = [m.copy() for m in doc["context"]]
        if msgs[-1]["role"] == "user" and not msgs[-1]["content"].strip().endswith("Правильный ответ:"):
            msgs[-1]["content"] = msgs[-1]["content"].rstrip() + " Правильный ответ:"
        return msgs

    def fewshot_context(self, doc, num_fewshot, *args, **kwargs):
        # zero-shot: just return the messages list

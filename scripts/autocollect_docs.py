...
def rename_manual_readme(dataset_dir, language):
    if not (
        dataset_dir / "README{}_manual.md".format("_ru" if language == "ru" else "")
    ).is_file():
        readme_old = dataset_dir / "README{}.md".format(
            "_ru" if language == "ru" else ""
        )
...
def format_data_field_desc(data_field_descriptions, lang, term_dict, data_ex):
    ...
    def get_field_desc(
        field_name, field_value, term_dict_value, lang, data_ex, level=0
    ):
        if lang in field_value:
            ...
            for sub_field, sub_field_value in field_value.items():
                if (
                    not lang in sub_field_value
                    or sub_field_value.get(lang, None) == "default"
                ):
    ...
    for field, value in data_field_descriptions.items():
        desc = get_field_desc(
            field, value, term_dict[field], lang=lang, data_ex=data_ex[field], level=0
        )
        desc_list.append(desc)
    ...
    def process_field(current_value, field_path):
        ...
        if isinstance(current_value, dict):
            ...
            for lang, val in current_value.items():
                if (
                    val == "default"
                    and "_desc" in current_dict
                    and lang in current_dict["_desc"]
                ):
...
def format_metrics(metrics_list, lang, term_dict):
    ...
    for metric, desc in metrics_list.items():
        metric_desc = (
            desc[lang]
            if desc[lang] != "default"
            else term_dict["metric_descriptions"][metric][lang]
        )
...
def format_hb(hb_results, hb_method, lang):
    ...
    if hb_method:
        hb_method_template = (
            hb_method_template_ru if lang == "ru" else hb_method_template_en
        )
        ...
        if hb_results:
            ...
            hb_res_str = hb_res_template.format(hb_res_str="\n".join(hb_res_list))
...
def complete_template(dataset_dir, language, meta, custom, term_dict):
    ...
    computed["data_field_descriptions"] = format_data_field_desc(
        ...
        lang=language,
        term_dict=term_dict["data_field_descriptions"],
        data_ex=meta["data_example"],
    )
    computed["data_example"] = json.dumps(
        meta["data_example"], indent=4, ensure_ascii=False
    )
    computed["metrics"] = format_metrics(
        meta["metrics"], lang=language, term_dict=term_dict
    )
    computed["human_benchmark"] = format_hb(
        meta["human_benchmark"], custom["Human baseline"], lang=language
    )
...
def autocollect_cards_and_meta(dataset_dir):
    ...
    for language in languages:
        ...
        complete_template(
            dataset_dir, language, meta=meta, custom=custom, term_dict=term_dict
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dataset_dir",
        type=str,
        help="dataset directory name inside `datasets/`, e.g. 'dataset_name'",
    )
